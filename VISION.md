## ChristianClaw Vision

ChristianClaw is OpenClaw aligned with Christian values and shipped as a
multi-agent team that serves one another in love.
It runs on your devices, in your channels, with your rules, and with a posture
of love, truth, grace, humility, and stewardship.
...
ChristianClaw is a focused fork of OpenClaw. OpenClaw started as a personal
playground to learn AI and build something genuinely useful: an assistant that
can run real tasks on a real computer. It evolved through several names and
shells: Warelay -> Clawdbot -> Moltbot -> OpenClaw -> ChristianClaw.

The goal: a Christian-values personal assistant and team that is easy to use,
supports a wide range of platforms, and respects privacy and security.

Christian-values posture and the default team are documented in
[`docs/start/christian-values.md`](docs/start/christian-values.md). The
`openclaw` CLI, config paths, and plugin namespace are retained for
compatibility; the product brand is ChristianClaw.
